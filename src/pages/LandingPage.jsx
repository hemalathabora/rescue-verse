...
import { FaUserShield, FaGlobeAsia, FaHandsHelping, FaFacebook, FaTwitter, FaInstagram, FaBolt } from "react-icons/fa";
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        <div className="relative z-10 text-center px-2 sm:px-6 max-w-6xl mx-auto w-full">
          <div className="animate-float">
            <h1 className="text-4xl sm:text-6xl md:text-8xl font-bold mb-8 text-white drop-shadow-2xl">
              Rescue
              <span className="bg-gradient-to-r from-violet-400 to-purple-600 bg-clip-text text-transparent">Verse</span>
              <span className="ml-2 sm:ml-4 text-3xl sm:text-5xl animate-pulse">🌍</span>
            </h1>
          </div>
          ...
          <p className="text-base sm:text-xl md:text-2xl mb-8 sm:mb-12 max-w-4xl mx-auto text-gray-200 leading-relaxed animate-fade-in-up">
            ...
            Get instant disaster updates, request critical resources, and coordinate
            emergency response seamlessly.
          </p>
          ...
          <div className="bg-gray-800/80 backdrop-blur-md rounded-2xl p-3 sm:p-4 border border-gray-600">
            <div className="flex items-center space-x-3 text-white">
              ...
              <div className="flex-1">
                <p className="text-xs sm:text-sm font-medium">{updates[currentUpdate]?.text}</p>
                <p className="text-xs text-gray-400">{updates[currentUpdate]?.time}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Marquee for reported disasters */}
            ...
            >
              {reportedDisasters.map((d, idx) => (
                <span key={idx} className="mx-8">
                  ...
                  <span>
                    <strong>{d.type}</strong> at <strong>{d.location}</strong>: {d.description}
                  </span>
                </span>
              ))}
            </marquee>
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-6">
          <div className="text-center mb-10 sm:mb-16">
            ...
            <p className="text-base sm:text-xl text-gray-300 max-w-3xl mx-auto">
              Real-time information about ongoing emergencies and disaster response efforts
            </p>
          </div>
              ...
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div className="flex items-center space-x-3 sm:space-x-4">
                    ...
                    <div>
                      <p className="text-base sm:text-lg font-medium text-white">{update.text}</p>
                      <p className="text-xs sm:text-sm text-gray-400">{update.time}</p>
                    </div>
                  </div>
